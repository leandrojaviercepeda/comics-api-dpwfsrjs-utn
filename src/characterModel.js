const { pool, getSqlFilter, parseArrayJson, getCols } = require('./DB');
const tableName = 'charact';
...
function getById(id=undefined) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            let query;
            if (id) query = `select * from ${tableName} where id_charact="${id}" limit 1;`;
            const response = await pool.query(query);
...
function del(id) {
    return new Promise(async (resolve, reject) => {
        try {
            if (!id || id == '') return reject({message: "Invalid request."});
            const query = `delete from ${tableName} where id_charact=?`;
...
function update(id, obj) {
    return new Promise(async (resolve, reject) => {
        try {
            if (!id || id == '') return reject({message: "Invalid request."});
            let query = `select * from ${tableName} where id_charact=? limit 1`;
            const oldObj = await getById(id);
            if (!oldObj) return reject({message: `Error al obtener ${tableName}.`});
            let newObj = {...oldObj};
            for (const key of Object.keys(obj)) {
                newObj[key] = obj[key];
            };
            delete newObj.id_charact;
            query = `update ${tableName} set ${Object.keys(newObj).map((key) => `${key}=${!Array.isArray(newObj[key]) ? JSON.stringify(newObj[key]) : `'${JSON.stringify(newObj[key])}'`}`)} where id_charact=?;`;
            resolve(pool.query(query, [id]));
        } catch (error) {
            console.error(error);
